...
public class PopUpTexter 
{
    ...
    [SerializeField] float _sineMagnitude;
    [SerializeField] float _sineStep;
     ...
     public void SpawnPopUp(PopUpText_Config config)
    {
        ...
        switch (_repeatPattern)
        {
            case TextSpawnRepeatPatterns.PingPong:
                value.x = Mathf.Sin(_currentSinePlace) * _sineMagnitude;
                _currentSinePlace += _sineStep;

                break;
            case TextSpawnRepeatPatterns.LoopRight:
                if (_currentSinePlace >= _sineMagnitude)
                    _currentSinePlace = _sineMagnitude * -1f;
            ...
            case TextSpawnRepeatPatterns.LoopLeft:
                if (_currentSinePlace <= _sineMagnitude *-1f)
                    _currentSinePlace = _sineMagnitude;
